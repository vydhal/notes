...
export function NewNoteCard ({onNoteCreated}: NewNoteCardProps){
  ...
  function handleStartRecording () {
    ...
    const isSpeechRecognitionAPIAvailable = 'SpeechRecognition' in window
     ...
     if (!isSpeechRecognitionAPIAvailable) {
       alert('O seu navegador não suporta a API de reconhecimento de fala.')
       return }
  ...
  function handleStopRecording () {
    ...
    return (
      <Dialog.Root   >
        <Dialog.Trigger className="rounded-md flex flex-col text-left  bg-slate-700 p-5 gap-3 outline-none hover:ring-2 hover:ring-slate-600  focus-visible:ring-2  focus:ring-lime-400">
      ...
      <Dialog.Overlay  className="inset-0  fixed bg-black/50"   />
        ...
        { shouldShowOnboarding ? (
             ...
             Comece <button type="button"  onClick={handleStartRecording} className="font-medium text-lime-400 hover:underline">gravando uma nota em aúdio</button> ou se preferir  <button type="button" onClick={handleStartEditor} className="font-medium text-lime-400 hover:underline">utilize apenas texto.</button>
           </p>
        ) : (
          <textarea autoFocus 
